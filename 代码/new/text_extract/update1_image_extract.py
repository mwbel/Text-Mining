...
def filter_fig_rect(page,fig_rect):
    if len(fig_rect)>1:
        ...
        for rect in fig_rect:
            ...
            if not len(wordsup):
                if len(wordsleft):
                    if wordsleft[0][4][0]=='图' or wordsleft[0][4][0]=='表':
...
def get_figrect_0(page,fignum,tablenum):
    ...
    # table_rects=[]
    ##搜'图 1'
    fig_rect=page.searchFor('图 '+str(fignum))
    fig_rect=filter_fig_rect(page,fig_rect)
    if len(fig_rect)>0:
        while len(fig_rect)>0:
            ...
            fig_rect=page.searchFor('图 '+str(fignum))
            fig_rect=filter_fig_rect(page,fig_rect)
    table_rect=page.searchFor('表 '+str(tablenum))
    fig_rect=filter_fig_rect(page,fig_rect)
    if len(table_rect)>0:
        while len(table_rect)>0:
            # print('表 ',tablenum,len(table_rect))
            fig_rects.append(['表 '+str(tablenum),table_rect[0]])
            tablenum+=1
            table_rect=page.searchFor('表 '+str(tablenum))
            fig_rect=filter_fig_rect(page,fig_rect)
    ##搜'图1'
...
if __name__ == "__main__":
    file_path="E:/项目/Demo/试运行/201808/中银国际"
    # file_path="D:/test"
    out_path="E:/pic/1/中银国际/"
...
# if __name__ == "__main__":
    # company_namelists = ['民生证券','安信证券','财通证券','东北证券', '东方财富','东方证券', '东吴证券','东兴证券','方正证券','光大证券',
    # '广发证券', '广证恒生', '国海证券','国金证券','国联证券','国盛证券', '国泰君安','国信证券','海通证券','恒大研究院','华创证券','华金证券',
    # '华泰证券','基业常青经济研究院','联讯证券','民生证券','莫尼塔投资', '平安证券','山西证券', '上海证券','申万宏源', '太平洋','天风证券',
    # '西部证券','新时代证券','信达证券','兴业证券','长城证券', '招商证券','中国科学院','中国信通院','中国银河','中航证券', '中金公司',
    # '中商产业研究院','中泰证券','中信建投','中信证券', '中银国际']
    # broker_lists = classify_file('E:/项目/Demo/试运行-2/试运行/201808', 'E:/项目/newRequest/券商名称')
    # for broker_list in broker_lists:
        # print(key)
        # file_path=os.path.join("E:/项目/Demo/试运行-2/试运行/201808",broker_list)
        # print(file_path)
        # file_path="D:/test"
        # out_path=os.path.join("E:/pic/1",broker_list)
        # failure_path = "E:/pic/1/未处理完成"
        # pdf_files = [name for name in os.listdir(file_path) if name.endswith('.pdf')]
        # print(pdf_files)
    #     for pdf in pdf_files:
    #         try:
    ...
    #             if not broker_arg or not len(broker_arg):
    #                 shutil.copy(os.path.join(file_path,pdf),failure_path)
    #             out_path=os.path.join(out_path,os.path.splitext(os.path.basename(pdf_path))[0])
    #             if not os.path.exists(out_path):
    #                 os.makedirs(out_path)
    #             shutil.copy(pdf_path,out_path)
    ...
    #             if os.path.isdir(out_path) and len(pdf_and_fig_lists) == 1 :
    #                         shutil.move(os.path.join(out_path,pdf),failure_path)
