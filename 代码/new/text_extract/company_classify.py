...
def classify_file(path, name_file):
    ...
    for name in pdf_files:
        ...
        for name_list in name_lists:
            ...
            if len(title_name) == 1 and name_list == title_name[0]:
                if not os.path.exists(output_path):
                    os.makedirs(output_path)
                # shutil.copy(os.path.join(path, name), output_path)
            ...
            elif len(title_name) > 1 and (name_list == title_name[0] or name_list == title_name[1] or name_list == title_name[2]):
                if not os.path.exists(output_path):
                    os.makedirs(output_path)
                # shutil.copy(os.path.join(path, name), output_path)
                if not os.path.exists(os.path.join(output_path, name)):
                ...
                continue
    return out_namelists,os.path.join(path,'按券商分类')
...
if __name__=='__main__':
    ...
    # classify_file(r'E:/项目/Demo/试运行-2/试运行/201808', r'E:/项目/newRequest/券商名称')